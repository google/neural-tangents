...
@layer
def ReluFeatures2(method: str = 'RANDFEAT',
                 feature_dim0: int = 1,
                 feature_dim1: int = 1,
                 sketch_dim: int = 1,
                 poly_degree: int = 8,
  ...
  def init_fn(rng, input_shape):
    ...
    if method == ReluFeaturesMethod.POLYSKETCH:
      new_nngp_feat_shape = nngp_feat_shape[:-1] + (poly_sketch_dim,)
      rng1, rng2 = random.split(rng, 2)
...
import numpy as onp
@layer
def NormGaussFeatures(method: str = 'RANDFEAT',
                 feature_dim0: int = 1,
                 feature_dim1: int = 1,
                 sketch_dim: int = 1,
                 poly_degree: int = 8,
  ...
  def init_fn(rng, input_shape):
    ...
    if method == ReluFeaturesMethod.RANDFEAT:
      ...
      kappa1_coeff = np.exp(-1) * np.array([1/np.exp(jax.scipy.special.gammaln(i+1)) for i in range(poly_degree+1)])
      kappa0_coeff = np.exp(-1) * np.array([1/np.exp(jax.scipy.special.gammaln(i+1)) for i in range(poly_degree+1)])
      ...
      # print(f"new_nngp_feat_shape : {new_nngp_feat_shape}")
      # print(f"new_ntk_feat_shape : {new_ntk_feat_shape}")
      return (new_nngp_feat_shape, new_ntk_feat_shape,
              new_net_shape), (polysketch, tensorsrht, kappa0_coeff,
                               kappa1_coeff)
  ...
  def feature_fn(f: Features, input=None, **kwargs) -> Features:
    ...
    # if np.any(np.isnan(nngp_feat_2d)) or  np.any(np.isnan(ntk_feat_2d)):
    #   print("1. nan is observed")
    ...
    if method == ReluFeaturesMethod.RANDFEAT:  # Random Features approach.
      ...
      # print(f"ntk_feat_2d avg norm : {np.sum(ntk_feat_2d.real**2 + ntk_feat_2d.imag**2, axis=-1).mean()}")
      # print(f"kappa0_feat avg norm : {np.sum(kappa0_feat.real**2 + kappa0_feat.imag**2, axis=-1).mean()}")
      # print(f"ntk_feat avg norm : {np.sum(ntk_feat.real**2 + ntk_feat.imag**2, axis=-1).mean()}")
    
      # aa = np.fft.fftn(ntk_feat_2d * tensorsrht.rand_signs1, axes=(-1,))[:, tensorsrht.rand_inds1]
      # bb = np.fft.fftn( (kappa0_feat * tensorsrht.rand_signs2), axes=(-1,))[:, tensorsrht.rand_inds2]
      # bb = np.fft.fftn(kappa0_feat * tensorsrht.rand_signs2, axes=(-1,))[:, tensorsrht.rand_inds2]
      # ntk_feat = np.sqrt(1 / tensorsrht.rand_inds1.shape[-1]) * (aa * bb).reshape(input_shape + (-1,))
      # if np.any(np.isnan(ntk_feat)) or np.any(np.isnan(nngp_feat)):
        # kk = kappa0_feat * tensorsrht.rand_signs2
        # np.sum(kk.real **2 + kk.imag**2, axis=-1)
        # bk = np.fft.fftn(kk, axes=(-1,))
        # np.sum(bk.real **2 + bk.imag**2, axis=-1)
        # qq = np.fft.fftn(kappa0_feat * tensorsrht.rand_signs2, axes=(-1,))
        # np.sum(qq.real **2 + qq.imag**2, axis=-1)
        # bb = np.fft.fftn( (kappa0_feat * tensorsrht.rand_signs2), axes=(-1,))
        # np.sum(bb.real**2 + bb.imag**2, axis=-1)
        # np.sum(ntk_feat_2d.real ** 2 + ntk_feat_2d.imag**2, axis=-1)
        # np.sum(kappa0_feat.real ** 2 + kappa0_feat.imag**2, axis=-1)
        # np.sum(kk.real ** 2 + kk.imag**2, axis=-1)

        # np.sum(kappa0_feat.real ** 2 + kappa0_feat.imag**2, axis=-1)
        
        # cc = kappa0_feat * tensorsrht.rand_signs2
        # ee = np.fft.fftn(cc, axes=(-1,))
        # enorm = np.sum(ee.real**2 + ee.imag**2, axis=-1)

        # print(cc.sum())
        # print("1.5. nan is observed")
        # import pdb; pdb.set_trace();
      # else:
        # print("sdfdsfs")
        # print(cc.sum())
    ...
    # if np.any(np.isnan(nngp_feat)) or  np.any(np.isnan(ntk_feat)):
    #   print("2. nan is observed")
    #   import pdb; pdb.set_trace();
