...
class PolyTensorSketch:
    ...
    def init_sketch(self) -> 'PolyTensorSketch':
        ...
        for i in range((self.degree - 1).bit_length()):
            ...
            if i == 0:
                ...
                tree_rand_inds[i] = random.choice(rng2,
                                              ske_dim_,shape=(deg_, 2, ske_dim_))
            deg_ = deg_ // 2
    ...
    def sketch(self, x):
        ...
        #E1 = np.concatenate((np.ones((n, 1), dtype=x.dtype), np.zeros((n, x.shape[-1] - 1), dtype=x.dtype)),1)
        ...
        for j in range(1, len(U)):
          ...
          for i in range(log_degree):
            if j % (2**(i + 1)) == 0:
                SetE1.add((i,p))
              #V[i] = V[i].at[p, :, :].set(np.concatenate((np.ones((n, 1)), np.zeros((n, V[i].shape[-1] - 1))), 1))
            else:
                if i == 0:
                    V[i] = V[i].at[p, :, :].set(
